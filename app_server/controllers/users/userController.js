 ...
 * Fecha: Mayo 2023
 * Path: app_server\controllers\users\userController.js
 * Descripción: Controlador para el sistema de usuarios.
 */
...
async function updateUserHandler(req, res) {
  ...
  const usuarioActualizado = await prisma.usuario
    .update({
...
async function deleteUserHandler(req, res) {
  ...
  const usuarioEliminado = await prisma.usuario
    .delete({
      where: { id_usuario: id_usuario },
    })
    .then(async function () {
...
async function getUserHandler(req, res, next) {
  ...
  const datos = await prisma.usuario
    .findMany({
      select: {
        id_usuario: true,
        nickname: true,
...
async function getLogrosHandler(req, res, next) {
  ...
  const datos = await prisma.logros
    .findUnique({
      select: {
...
async function getRankingPartidasHandler(req, res, next) {
  try {
    ...
    for (const registro of ranking) {
      ...
      if (nickname) {
        // El valor existe en el diccionario
...
async function getEstadisticasHandler(req, res, next) {
  ...
  const datos = await prisma.estadisticasacumuladas
    .findUnique({
      select: {
        vecesoca: true,
        vecesseis: true,
